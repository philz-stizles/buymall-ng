...
export class AuthService {
  ...
  preventUnauthenticated(allowedRoles: string[], returnUrl: string) {
    return this.user.pipe(
      take(1),
      map((user) => {
        const isAuthenticated = !!user;
        if (isAuthenticated) {
          if (allowedRoles.length > 0 && user.isAuthorized(allowedRoles)) {
  ...
  preventAuthenticated() {
    return this.user.pipe(
      take(1),
      map((user) => {
        console.log('preventAuthenticated', user);
        const isAuthenticated = !!user;
        if (isAuthenticated) {
          return this.router.createUrlTree(['/']);
        }
